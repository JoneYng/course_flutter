...
class OpenClassButtonView extends StatelessWidget {
  ...
  Widget getWidget(OpenClassState mOpenClassState, BuildContext context) {
    if (mOpenClassState == OpenClassState.NotStarted) {
      return OutlineButton(
        borderSide: BorderSide(color: Colours.open_class_button, width: 1),
        child: Text(
          "开播提醒",
          style: TextStyle(color: Colours.open_class_button),
        ),
        onPressed: () {
          Toast.show("直播未开始~");
        },
  ...
  OpenClassState getButtonState(OpenClassEntity mOpenClassEntity) {
    ...
    } else if (nowMilliseconds > mOpenClassEntity.stime &&
        nowMilliseconds < mOpenClassEntity.etime) {
